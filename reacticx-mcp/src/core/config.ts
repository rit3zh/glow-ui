...
export function validate(config: ComponentConfig): string[] {
  ...
  if (!POLICIES.includes(config.installDependencies)) {
    problems.push(`"installDependencies" must be one of ${POLICIES.join(", ")}`);
  }
  ...
  for (const key of ["components", "utils", "hooks"] as const) {
    if (typeof config.aliases[key] !== "string") {
...
export function serializeConfig(config: ComponentConfig) {
...
export async function writeConfig(config: ComponentConfig, root: string) {
  const filePath = configPath(root);
  await fs.writeFile(
    filePath,
